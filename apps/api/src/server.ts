...
export async function createServer(){ const config=loadConfig(); const server=Fastify({ logger:false, serializerOpts:{ rounding:'ceil' }, ajv:{ customOptions:{ removeAdditional:false } } });
 ...
 server.setErrorHandler(errorHandler); await registerRequestId(server); await registerAuth(server); await registerApiVersion(server,config); await registerRateLimit(server); await registerIdempotency(server); await registerAudit(server); await server.register(v1Routes,{prefix:'/v1'}); return server; }
