...
| P1 Daml Model | PARTIAL | Daml packages under `daml/`; schema/templates declared; package tests previously reported as arithmetic/unit checks. | Templates declared and schema correct, but Hold/Holding atomicity bug pending; tests are arithmetic, not ledger-script lifecycle proof. |
| P2 API Contract | BROAD PROTOTYPE | `packages/api-contracts/openapi/pillar-v1.yaml`; generated JSON/golden tests previously reported exit 0; API route files exist. | Schemas, OpenAPI, and golden fixtures are present; many routes return fixture/fallback objects. |
| P3 DB/Idempotency | PARTIAL | `packages/db/migrations/0000_*..0130_*`; migrator verify previously reported exit 0; idempotency package present. | Migrations exist and verify works; runtime integration has a dual in-memory/DB idempotency path that must converge to DB-only. |
...
| P5 Projection/Reconciliation | PARTIAL | Projection routes/repositories in `apps/api/src/routes/v1/projection.ts`; migrations and services present. | Services compile and migrations exist; projection repo currently returns fallback demo rows on miss, violating IC-02. |
