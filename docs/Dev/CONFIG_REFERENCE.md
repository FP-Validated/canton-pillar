...
| `PILLAR_USAGE_BILLING_PROVIDER` / `usageMeter.billingProvider` / `--usage-billing-provider`                                    | enum       | `billing` | Yes       | Billing provider adapter.                            | `P14.Q04`     |
| `PILLAR_BILLING_API_KEY_SECRET_REF` / `usageMeter.billing.apiKeySecretRef` / `--billing-api-key-secret-ref`                       | Secret ref | none     | Mode      | Billing API key reference.                            | `P14.Q04`     |
| `PILLAR_BILLING_WEBHOOK_SECRET_REF` / `usageMeter.billing.webhookSecretRef` / `--billing-webhook-secret-ref`                      | Secret ref | none     | Mode      | Billing webhook verification secret reference.        | `P14.Q04`     |
...
| `PILLAR_BILLING_ACCOUNT_MODE` / `global.billing.accountMode` / `--billing-account-mode`                                                 | enum       | `test`           | Mode      | Billing account mode for billing integration.                          | `P14.Q04`     |
...
| Billing       | Pillar billing integration.                                        | Usually Pillar billing integration with customer usage source.       | Optional or customer billing integration.                                   | Usage events are auditable regardless of billing provider. |
...
| `services/usage-meter`              | `/healthz/config`             | `usageMeter`             | Billing key refs, usage DB URL.                                                       | Usage DB, billing provider when enabled, export bucket.        |
