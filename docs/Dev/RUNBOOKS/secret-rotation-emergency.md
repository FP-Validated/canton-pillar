...
| Customer-visible symptoms | Authentication failures, webhook signature changes, revoked sessions, delayed KYC/Stripe workflows, mTLS reconnects, template deployment pause, or emergency customer cutover notice.                                                                                                                |
...
| Is public `/v1` behavior affected?                                          | Include API owner; preserve Stripe-like grammar and idempotency semantics.                              |
...
| Stripe API key      | Rotate in Stripe dashboard; update `PILLAR_STRIPE_API_KEY_SECRET_REF`; restart only dependent payment/billing worker if required.       | Billing/payment flows may pause; no ledger mutation shortcut.                             | Third-party payment integration isolation.                   |
...
| Stripe API key      | Rotate in Stripe dashboard; store new secret in secret manager; update `PILLAR_STRIPE_API_KEY_SECRET_REF`; reload billing/payment worker; verify Stripe auth and webhook/payment flows.                                            | Posting key into Helm values/tickets, mixing test/live keys, retrying charges outside idempotent Stripe semantics.                                       | Pause Stripe-dependent flows; no ledger mutation shortcut for payment uncertainty.                                                                           |
...
| Resume paused workflows          | Webhook dispatcher, onboarding/KYB, billing/Stripe, DAR promotion, or ledger-command queues resume only after targeted smoke tests pass.                |
...
| Stripe API key      | Stripe test API call succeeds with new ref; no live/test key mix; idempotent payment/billing checks pass.                                   |
...
| Stripe API key      | `pillarctl billing stripe-smoke --env prod --no-charge`                                 | Stripe authentication succeeds; no mutation outside idempotent/no-charge check.                                                 |
