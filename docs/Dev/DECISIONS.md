...
| [ADR-0002](#adr-0002-billing-style-external-api-surface-canton-internals-hidden)                                       | Accepted | REST-style external API surface; Canton internals hidden                                            | -          | [P2](./Phase_02_API_Contract.md), [P4](./Phase_04_Ledger_Command_Runtime.md), [P7](./Phase_07_SDK_CLI_Workbench.md), [P8](./Phase_08_Security_Compliance.md)                                                                                                                                                                                                                                                                           |
...
### ADR-0002: REST-style external API surface; Canton internals hidden

- Status: Accepted
- Context: The mission is to ship Pillar as the "The payments runtime for Canton-backed assets." Invariants require that the public `/v1` API never exposes `contractId`, `templateId`, `partyId`, `participantId`, `packageId`, or `commandId`, while every mutation remains internally traceable through `intent → operation → command_id → update_id/offset`.
- Decision: Pillar's public API is REST-style REST over stable public objects and IDs; Canton and Daml identifiers remain internal runtime/audit details unless exposed through a privileged trace surface.
- Consequences:
  ...
  - [Phase_07_SDK_CLI_Workbench.md](./Phase_07_SDK_CLI_Workbench.md) must preserve the polished developer model in SDKs, CLI, and Workbench.
...
- Status: Accepted
- Context: Invariant 7 requires webhooks to be "signed (HMAC-SHA256), versioned per endpoint, retried, replayable." Architecture/10 adopts billing's webhook model with thin events, HMAC-SHA256 over `{timestamp}.{raw_payload}`, endpoint-specific secrets, replay handling, and per-endpoint payload versions.
