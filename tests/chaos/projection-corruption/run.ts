...
export async function run(): Promise<ScenarioReport> {
  const scenario = "projection-corruption"; const report: ScenarioReport = { scenario, status: "passed", startedAt: now(), finishedAt: now(), invariants: {}, samples: {}, notes: [] };
  ...
  const projection = await sampleProjection(true); const webhooks = await sampleWebhooks(false); unpauseContainer(scenario); compose(["down", "--remove-orphans"]);
