...
export async function run(): Promise<ScenarioReport> {
  const scenario = "webhook-mass-failure"; const report: ScenarioReport = { scenario, status: "passed", startedAt: now(), finishedAt: now(), invariants: {}, samples: {}, notes: [] };
  ...
  const projection = await sampleProjection(false); const webhooks = await sampleWebhooks(true); unpauseContainer(scenario); compose(["down", "--remove-orphans"]);
